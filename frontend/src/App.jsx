...
function App() {
  ...
  const handleAuth = async (e) => {
    ...
    if (isRegistering) {
      ...
      // store encrypted private key (encrypted using user password)
      const wrappedPrivKey = await encryptData(privKeyRaw, creds.password);

      const res = await api('register', 'POST', {
        ...creds,
        publicKey: pubKey,
        privateKey: JSON.stringify(wrappedPrivKey)
      });
    ...
    // ✅ LOGIN: request keys from backend
    const res = await api('login?includeKeys=true', 'POST', creds);
    if (res && res.ok) {
      const data = await res.json();
      setToken(data.token);

      // ✅ prevent crash if keys missing
      if (!data.privateKey || !data.publicKey) {
        showToast("❌ Keys not received from server", "error");
        return;
      }
  ...
  const handleDecryptAll = async () => {
    ...
    if (!keyPair?.publicKey) return showToast("⚠️ Login keys missing", "error");
  ...
  const handleSave = async () => {
    ...
    if (editingId) {
      // ✅ UPDATE MODE (must use PATCH to match backend)
      const res = await api(`notes/${editingId}`, 'PATCH', {
        title: newTitle || "Doc",
        ...encrypted,
        signature
      });
    ...
    <div className="container" style={{ maxWidth: '450px' }}>
      ...
      <div className="glass-panel" style={{ textAlign: 'center' }}>
        ...
        <button className="secondary" onClick={() => setIsRegistering(!isRegistering)}>
          {isRegistering ? "BACK" : "CREATE ACCOUNT"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 30 }}>
        ...
        <button onClick={handleLogout} style={{ width: 'auto', color: 'var(--neon-pink)' }}>
      ...
      <div className="glass-panel" style={{ marginBottom: 30, padding: '1.5rem' }}>
        ...
        />
        <button onClick={handleDecryptAll}>
        ...
        {notes.map(n => (
          <div key={n._id} className="note-card" style={{ borderColor: editingId === n._id ? 'var(--neon-green)' : '' }}>
            ...
            <div style={{ marginTop: 15, display: 'flex', gap: '10px' }}>
              <button onClick={() => handleDelete(n._id)} className="icon-btn" style={{ padding: '5px' }}>
                <TrashIcon />
              </button>
              ...
              {n.isDecrypted && (
                <button
                  ...
                  className="icon-btn"
                  style={{ padding: '5px', color: 'var(--neon-green)', borderColor: 'var(--neon-green)' }}
                >
                  <EditIcon />
                </button>
              )}
